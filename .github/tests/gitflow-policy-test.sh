...
MOCK_COMPANION_SHA=backmerge-tip
...
expect_status 0 "release with companion targets main"

reset_mocks
BASE_REF=main HEAD_REF=release/1.0.0 MOCK_COMPANION_SHA=""
expect_status 1 "release without companion is blocked"

reset_mocks
BASE_REF=main HEAD_REF=release/1.0.0 MOCK_COMPANION_SHA=""
MOCK_ANCESTORS="approved:target-tip"
...
expect_status 0 "hotfix without release uses develop companion"

reset_mocks
BASE_REF=main HEAD_REF=hotfix/1.0.1 MOCK_ACTIVE_RELEASES=release/2.0.0
expect_status 0 "hotfix uses active release companion"
